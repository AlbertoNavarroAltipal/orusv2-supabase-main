...
import React, { useState, useEffect, memo, useMemo } from "react"; // Añadido useMemo
import {
  Table,
  TableBody,
  TableCell,
...
import { plantillaConfig } from "../config"; // Importar plantillaConfig
...
const DataTableComponent: React.FC<DataTableProps> = ({
  ...
  // `allPossibleColumns` ahora se toma de plantillaConfig.
  // No es necesario definirlo aquí de nuevo.
  // const allPossibleColumns: { accessorKey: keyof UserData; header: string }[] = plantillaConfig.allTableColumns;

  const displayedColumns = useMemo(() => {
    // Asegurarse de que plantillaConfig.allTableColumns tiene el tipo correcto o hacer un cast si es necesario.
    // En este caso, la interfaz TableColumnConfig en config.ts ya usa accessorKey.
    return plantillaConfig.allTableColumns.filter((col) =>
      visibleColumns.includes(col.accessorKey as keyof UserData)
    );
  }, [visibleColumns]); // plantillaConfig.allTableColumns es estable, no necesita estar en las dependencias si no cambia en runtime
  ...
  const rowClasses = (rowIndex: number) => {
    ...
    if (stripedRows && rowIndex % 2 !== 0) {
      // 0-indexed, so odd index is even row visually
    ...
    <div className="flex items-center justify-between p-4 border-t">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Button
          ...
          disabled={
            currentPage === totalPages || totalPages === 0 || totalPages < 1
          }
        >
  ...
  return (
    <div className="bg-white">
      ...
      <div className="overflow-x-auto">
        {" "}
        {/* Contenedor para scroll horizontal si es necesario y para el sticky header */}
        <Table className={tableClasses}>
          <TableHeader
            className={`${stickyHeader ? "sticky top-0 z-10" : ""} bg-white ${
              !showGridLines ? "border-b-0" : ""
            }`}
          >
            <TableRow className={!showGridLines ? "border-b-0" : ""}>
              {enableRowSelection && (
                <TableHead
                  className={`w-[40px] ${currentDensityClass} ${
                    !showGridLines ? "border-0" : ""
                  } ${currentTextAlignClass}`}
                >
                  <Checkbox
                    checked={selectAll}
                ...
                >
                  {enableSorting ? (
                    <Button
                      variant="ghost"
                      onClick={() =>
                        onSortChange(column.accessorKey as keyof UserData)
                      }
                    >
                      {column.header}
          ...
          <TableBody className={!showGridLines ? "divide-y-0" : ""}>
            {isLoading ? (
              ...
              <TableRow className={!showGridLines ? "border-b-0" : ""}>
                <TableCell
                  colSpan={
                    displayedColumns.length + (enableRowSelection ? 1 : 0)
                  }
                  className={`h-24 text-center ${currentDensityClass} ${
                    !showGridLines ? "border-0" : ""
                  }`}
                >
                  No hay resultados.
